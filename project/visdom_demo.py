...
def run_demo(viz):
    ...
    img_callback_win = viz.image(
        np.random.rand(3, 512, 256), opts={"title": "Random!", "caption": "Click me!"},
    )
    ...
    viz.scatter(
        ...
        opts=dict(textlabels=["Label %d" % (i + 1) for i in range(10)]),
    )
    viz.scatter(
        X=np.random.rand(10, 2),
        Y=[1] * 5 + [2] * 3 + [3] * 2,
        opts=dict(
            legend=["A", "B", "C"], textlabels=["Label %d" % (i + 1) for i in range(10)]
        ),
    )
    ...
    win = viz.line(
        ...
        opts={
            ...
            "linecolor": np.array([[0, 191, 255], [0, 191, 255], [255, 0, 0],]),
    ...
    viz.line(
        ...
        name="4",
        update="insert",
        opts={"linecolor": np.array([[255, 0, 0],]), "dash": np.array(["dot"]),},
    )
    ...
    viz.quiver(
        X=U, Y=V, opts=dict(normalize=0.9),
    )
    ...
    viz.scatter(
        X=np.random.rand(255),
        Y=np.random.rand(255),
        opts=dict(markersize=10, markercolor=colors[0].reshape(-1, 3),),
        name="1",
        update="append",
        win=win,
    )
    ...
    viz.scatter(
        X=np.random.rand(255, 2),
        Y=(np.random.rand(255) + 1.5).astype(int),
        opts=dict(markersize=10, markercolor=colors,),
        update="append",
        win=win,
    )
    ...
    try:
        ...
        viz.line(Y=torch.Tensor([[0.0, 0.0], [1.0, 1.0]]))
    except ImportError:
